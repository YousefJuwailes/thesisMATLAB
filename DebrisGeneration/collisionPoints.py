...
def collisionPoints(xyz, elevation, elevationZero, penetrationDistance):
    ...
    for i in range(0, xyz.shape[0], 10):
        if checkForCollision(xyz[i, :3], elevation, elevationZero) and np.linalg.norm(
    ...
    if j >= 9:
        for i in range(1, 10):
            if checkForCollision(xyz[j - i, :3], elevation, elevationZero):
